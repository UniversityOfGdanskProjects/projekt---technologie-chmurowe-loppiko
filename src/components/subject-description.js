...
function SubjectDescription({handleTransparentOnClick, database}) {
    const {semesterId, subjectName} = useParams();
    const currentSubject = database[semesterId].find(subj => subj["subject-name"] === subjectName.split("-").join(" "));
